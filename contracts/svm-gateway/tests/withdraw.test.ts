...
describe("Universal Gateway - Withdraw Tests", () => {
    ...
    describe("revert withdrawals", () => {
        ...
        it("rejects revert when signed gas_fee is below the measured gas_used (InsufficientGasBudget)", async () => {
            // Under the measured-reimbursement model, gas_fee is a signed ceiling and the on-chain
            // program refunds the measured cost. Signing a gas_fee smaller than the base measured
            // cost (SIGNATURE_FEE + ExecutedSubTx rent ≈ 895k lamports) trips the cap check.
        ...
        it("auto-creates recipient ATA on SPL revert to a fresh wallet and reimburses ATA rent", async () => {
            ...
            // Program creates it via ensure_associated_token_account, folds rent into measured
            // gas_used, and pays the caller from fee_vault. gas_fee is the signed ceiling.
            const revertTokens = 500;
            const revertRaw = BigInt(revertTokens) * TOKEN_MULTIPLIER;
            ...
            // FeeVault paid exactly the measured amount; caller nets ≈ 0 (rent + tx fee refunded).
        ...
        it("rejects SPL revert to a fresh wallet when gas_fee cannot cover ATA rent (InsufficientGasBudget)", async () => {
            ...
            const executedSubTxRent = await provider.connection.getMinimumBalanceForRentExemption(8);
            // Deliberately size gas_fee to cover only the base (SIG_FEE + ExecutedSubTx rent)
            // but NOT the added ATA rent. On-chain measured cost will exceed gas_fee → reject.

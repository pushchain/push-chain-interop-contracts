...
6. `FeeVault → Caller` (`gas_used`, UV reimbursement).
...
The event carries `gas_used` (the reimbursed amount) so Push can reconcile — an IDL-breaking layout addition; regenerate types.

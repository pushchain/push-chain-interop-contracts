...
The delegate check is a delta rather than an absolute so that prior legitimate delegations survive across later unrelated executes. On the current-mint ATA (`budget = staged amount`), `spl_token::Revoke` (allowance → 0) is allowed and self-delegation up to the staged amount by a target program is allowed. Attempts to install a standing delegate that exceeds the current tx's staged amount revert. On bystander CEA-owned ATAs (`budget = 0`), delegate identity and allowance must be strictly unchanged — `Revoke` on a bystander reverts, since the CPI target has no legitimate reason to touch delegations on accounts unrelated to the staged mint.

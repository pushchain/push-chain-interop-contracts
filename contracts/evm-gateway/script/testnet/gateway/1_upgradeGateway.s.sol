 ...
 * @notice Step 1 of 3: Deploy new UniversalGateway implementation and upgrade the proxy.
 *         Does NOT call initializeV2 — that is a separate step (2_initializeV2.s.sol).
 *
 * @dev    Reads gatewayProxy from GatewayConfig for the current chain.
 *         The caller MUST be the ProxyAdmin owner.
 *
 * USAGE:
 *   forge script script/testnet/gateway/1_upgradeGateway.s.sol:UpgradeGateway \
...
contract UpgradeGateway is Script, GatewayConfig {
    ...
    function run() external {
        ...
        console.log("========================================");
        console.log("  STEP 1: UPGRADE UniversalGateway");
        console.log("========================================");
    ...
    function _preValidate() internal view {
        ...
        require(cfg.gatewayProxy != address(0), "gatewayProxy not configured");

        uint256 sz;
        address p = cfg.gatewayProxy;
        assembly { sz := extcodesize(p) }
        require(sz > 0, "no code at gatewayProxy");
    ...
    function _snapshotState() internal {
        ...
        console.log("  INBOUND_FEE    :", gw.INBOUND_FEE());
        console.log("  paused         :", gw.paused());
        console.log("");
    }
    ...
    function _upgrade() internal {
        ...
        ProxyAdmin(proxyAdmin).upgradeAndCall(ITransparentUpgradeableProxy(cfg.gatewayProxy), newImpl, bytes(""));
